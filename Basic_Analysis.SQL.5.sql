...
select emp_id,emp_name, department_name from employees left join department on department.department_id = employees.department_id order by emp_id;  

#3. Write a query to get department details even if there are no employees in that department. 
SELECT department.department_id,department_name,COUNT(emp_id) FROM department
 LEFT JOIN employees ON department.department_id=employees.department_id GROUP BY department.department_name,department.department_id;
...
select  employees.emp_name,department.department_name,project.project_name from employees left join department on department.department_id = employees.department_id 
left join project on employees.emp_id = project.emp_id;

#6. Find employees who are not assigned to any projects.
select emp_name from employees left join project on employees.emp_id = project.emp_id where project_name is null;

#7. Find departments with no employees using a RIGHT JOIN. 
select department_name from employees right join department on department.department_id = employees.department_id where emp_name is null;
...
JOIN employees b ON a.department_id = b.department_id AND a.emp_id != b.emp_id;
...
select project.project_name,employees.emp_name from employees join project on employees.emp_id = project.emp_id
join department on department.department_id = employees.department_id where department_name = "IT";

#11. Write a query to show employees and their project information (even if not assigned to a project). 
select emp_name,project_name from employees left join project on employees.emp_id = project.emp_id;

#12. Find employees who work in departments with names starting with 'A'. 
select emp_name, department_name from employees
left join department on department.department_id = employees.department_id where department_name regexp '^A';
